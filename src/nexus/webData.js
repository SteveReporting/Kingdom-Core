...
function displayMember(member) {
  ...
  return {
    ...
    roles: member.roles?.cache
      ? [...member.roles.cache.values()].filter((role) => role.id !== member.guild.id).sort((a, b) => b.position - a.position).map((role) => role.name)
      : [],
...
function houseFromRoles(roles = []) {
  return roles.find((role) => /^.*house\s+/i.test(String(role)))?.replace(/^.*house\s+/i, '') ?? null;
}
...
export function buildIdentityPayload(guild, nexus, state) {
  ...
  const merged = members.map((member) => {
    ...
    return {
      ...
      robloxId: profile.robloxId ?? profile.roblox_id ?? null,
...
export function buildApplicationsPayload(state, guild) {
  ...
  const applications = values(state.applications).map((app) => {
    ...
    return {
      ...
      applicant: member?.user?.username ?? app.username ?? app.applicant ?? app.userId ?? 'Unknown',
      ...
      reviewer: review.claimedBy ?? app.reviewer ?? null,
  ...
  const pending = applications.filter((app) => ['pending', 'reviewing', 'interview'].includes(String(app.status).toLowerCase())).length;
  return { applications, pending, count: applications.length };
}
...
export function buildVaultPayload(nexus) {
  const backups = values(nexus.vault?.backups).sort((a, b) => new Date(b.createdAt ?? b.timestamp ?? 0) - new Date(a.createdAt ?? a.timestamp ?? 0));
  const verified = backups.filter((item) => item.verified === true || item.status === 'ok');
  return {
    backups,
    snapshots: backups,
    latest: backups[0]?.createdAt ?? backups[0]?.timestamp ?? null,
    oldest: backups.at(-1)?.createdAt ?? backups.at(-1)?.timestamp ?? null,
    ...
    integrityChecks: backups.slice(0, 25).map((item) => ({
      name: item.file ?? item.path ?? item.id ?? 'Snapshot',
      status: item.verified === true || item.status === 'ok' ? 'ok' : 'pending',
      at: item.createdAt ?? item.timestamp ?? null
    }))
  };
}

export function buildStudioPayload(nexus) {
  const layouts = values(nexus.studio?.layouts);
  return { layouts, data: layouts, count: layouts.length };
}

export function buildNetworkPayload(nexus, guild) {
  const tenants = values(nexus.network?.tenants);
  const active = tenants.filter((tenant) => String(tenant.status ?? 'active').toLowerCase() === 'active');
  return {
    tenants,
    guilds: Math.max(1, tenants.length),
    totalGuilds: Math.max(1, tenants.length),
    active: active.length,
    members: guild?.memberCount ?? null,
    totalMembers: guild?.memberCount ?? null,
